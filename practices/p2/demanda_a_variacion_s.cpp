/**
	@brief Este programa genera 3 ficheros; demanda_a_y_1_variacion_s.dat, demanda_a_y_5_variacion_s.dat, demanda_a_y_10_variacion_s.dat.dat
		    los cuales sirven para representar la ganancia esperada para cada valor de s y para distintos valores de y utilizando
			 la distribucion de probabilidad a) suministrada.
			 NOTA: El nº de veces para la simulacion se ha escogido 35000, ya que se mantiene estable el valor de la ganancia
					 para dicho valor, segun nos muestra los ficheros resultados del programa demanda_a_variacion_veces.cpp.
					 
					 Se ha escogido el siguiente valor de referencia para x:
						- x=10.
*/
#include <iostream>
#include <cstdlib>
...
int main(){
	...
	fo[0].open("./datos/demanda_a_y_1_variacion_s.dat");
	fo[1].open("./datos/demanda_a_y_5_variacion_s.dat");
	fo[2].open("./datos/demanda_a_y_10_variacion_s.dat");
	...
	for(s=1;s<=100;s++){
		for(int k=0;k<3;k++){
			y=array_y[k];
				...
				gananciaesperada=sum/35000;

				fo[k] << s << ' ' << gananciaesperada << endl;
